...
const CORE = ['./', './index.html', './logomain.jpg', './manifest.json',
              ...
              /* ⚠️ 2026-08-28：這裡原本寫著「筆記那頁的三支排版函式庫不能加進來，
                 addAll 對跨來源網址會整批失敗」，而且刻意不寫出它們的名字。
                 **那段話現在整段都不成立了**，留著會讓下一輪的自己以為
                 lucide／marked／DOMPurify 還掛在別人的網域上：

                   1. 2026-08-14 那一輪已經把它們全部搬進 ./vendor/ 自己 host，
                      同源之後「跨來源會整批失敗」這條限制就消失了。
                      三支裡有兩支（marked、DOMPurify）就在上面的 CORE 裡。
                   2. 唯一沒進來的是 KaTeX，理由**不是**跨來源，是體積
                      （js 加 20 個 woff2 字型約 570KB，而多數人開筆記頁不是為了寫公式）。
                      理由寫在上面 vendor 那一段。
                   3. 也不必再避開它的名字：`tests/latex.mjs` 早就改成
                      **剖析 CORE 陣列本身**再判斷，不再是「這個檔案裡不准出現那個名字」。
                      （會改就是因為註解裡的說明文字被誤傷過。）

                 唯一還活著的那半句是**降級路徑不能拿掉**：KaTeX 不在 CORE，
                 所以離線第一次開筆記頁時算式仍然只有純文字備援，
                 `tests/latex.mjs` 那整套「沒有 KaTeX 也要活著」就是在釘這件事。 */
...
/* ★ 內建單字庫（./data/vocab/*.json，約 3.8MB）**故意不放進 CORE**：
   ...
   那天從 1 科 51KB 變成 67 科約 6.9MB（隔一輪砍掉兩科，剩 65 科）。放進 CORE 的話，每個人第一次開站
